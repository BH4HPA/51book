...
async function main () {
  ...
  for (i = 0; i < ctbxhr.data.length; i++) {
    ...
    fs.writeFileSync('./docs/README.md', fs.readFileSync('./docs/README.md').toString() + `\n\n- <img style="height: 24px;float: left;margin-right: 10px;border-radius: 15px;" alt="${contributor.login}'s avatar" src="./.vuepress/public/github-avatars/${contributor.login}.svg"> [${contributor.login}](${contributor.html_url}) - ${contributor.contributions} 次贡献.`)
    fs.writeFileSync('./docs/联系我们.md', fs.readFileSync('./docs/联系我们.md').toString() + `\n\n- <img style="height: 24px;float: left;margin-right: 10px;border-radius: 15px;" alt="${contributor.login}'s avatar" src="./.vuepress/public/github-avatars/${contributor.login}.svg"> [${contributor.login}](${contributor.html_url}) - ${contributor.contributions} 次贡献.`)
  }
